...
public class BKDReader extends PointValues {
  ...
  private static class BKDPointTree implements PointTree {
    ...
    /** Prefetch doc IDs below current node for two-phase traversal. */
    public void prefetchDocIDs(TwoPhaseIntersectVisitor visitor) throws IOException {
      resetNodeDataPosition();
      prefetchAll(visitor, false);
    }

    /** Visit doc IDs for a leaf node at the provided input position. */
    ...
    /** Recursively prefetch all leaf blocks below this node. */
  ...
  /**
   * Two-phase intersect visitor for BKD tree traversal with prefetching.
   * Phase 1: recurse the tree, optionally triggering IO prefetch for matching leaf blocks.
   * Phase 2: visit the deferred (prefetched) blocks.
   *
   * @lucene.experimental
   */
  ...
  public abstract static class BaseTwoPhaseIntersectVisitor implements TwoPhaseIntersectVisitor {
    ...
    java.util.List<Long> deferredBlocks = new java.util.ArrayList<>();
    ...
    @Override
    public java.util.List<Long> deferredBlocks() {
      return new java.util.ArrayList<>(deferredBlocks);
    }
  ...
  /**
   * Override intersect to support two-phase BKD traversal with prefetching.
   * When the visitor is a TwoPhaseIntersectVisitor, phase 1 recurses the tree
   * prefetching matching leaf blocks, then phase 2 visits the deferred blocks.
   */
  @Override
  public void intersect(IntersectVisitor visitor) throws IOException {
    ...
    if (visitor instanceof TwoPhaseIntersectVisitor twoPhaseIntersectVisitor) {
      intersectTwoPhase(twoPhaseIntersectVisitor, pointTree);
      java.util.List<Long> fps = twoPhaseIntersectVisitor.deferredBlocks();
  ...
  private static void intersectTwoPhase(TwoPhaseIntersectVisitor visitor, BKDPointTree pointTree)
      throws IOException {
    while (true) {
      Relation compare =
          visitor.compare(pointTree.getMinPackedValue(), pointTree.getMaxPackedValue());
      if (compare == Relation.CELL_INSIDE_QUERY) {
        // This cell is fully inside the query shape: recursively prefetch all points
        pointTree.prefetchDocIDs(visitor);
      } else if (compare == Relation.CELL_CROSSES_QUERY) {
        // The cell crosses the shape boundary — recurse or visit leaf
